...
export class UIElementDateTimeField extends UIElement {
  static renderLabel(context: UIElementRenderContext, id: string, text: string) {
    return context.layouter.renderElementLabels
      ? html`
          <label for="${id}"
            >${text}
            ${this.devInfo(
              context,
              context.entry.element_type.text,
              context.entry.element_type.value,
            )}
          </label>
        `
        ...
        html`
          ...
          ${context.entry.element_type.readonly
            ? html`<div class="date-time-frame">
                ${timeZoneInfo
                  ? html`<i class="time-zone-marker" @click="${this.openDateTimePopover}">
                      <div class="date-time-popover" @click="${this.openDateTimePopover}">
                        ${timeZoneInfo ? timeZoneInfo.tz_long : "unspecified time zone"}
                      </div></i
                    > `
                  : nothing}
                <input
            ...
            : html`${timeZoneInfo ? html`<div class="date-time-frame"><i class="time-zone-marker"></i></div>` : nothing}
