...
export class UIElementTemplateLabel extends UIElement {
  static renderLabel(context: UIElementRenderContext, id: string, text: string) {
    return context.layouter.renderElementLabels && context.entry.element_type.text
      ? html`
          <label for="${id}"
            >${text}
            ${this.devInfo(
  ...
  static render(context: UIElementRenderContext, id: string) {
    ...
    this.addStyle(
      cssStyle,
      context.uicomponent
        .getPaddingStyle(context.entry.layout?.padding)
    ...
    return context.layouter.renderElement(
      ...
      html`
        ...
        <div class="templateLabel ${htmlClass}" id=${id} style="${cssStyle ? cssStyle : nothing}">
          ${this.devInfo(
            context,
            context.entry.element_type.text,
            context.entry.element_type.value,
