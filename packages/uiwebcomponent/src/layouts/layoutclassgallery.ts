...
export class UIGalleryLayoutClass extends UIListLayoutClass {
  ...
  public renderSortButton(
    ...
    if (orderIndex > -1) {
      ...
      return html` <div
        ...
        data-id="${id}"
        @click="${allAllowed || orderAllowed.includes(id)
          ? this.headerClicked.bind(this)
          : nothing}"
      >
        ${order.startsWith(">")
          ? html`<span class="${cssClass}"><i></i></span>`
          : html`<span class="${cssClass}"><i></i></span>`}
  ...
  public renderLayout(
    ...
    while (renderContext.next()) {
      items.push(html`
        <div
          id="R${renderContext.getCurrentUID() ? renderContext.getCurrentUID() : nothing}"
          class="gallery-item "
